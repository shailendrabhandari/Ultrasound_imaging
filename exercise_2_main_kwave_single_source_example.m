...
number_of_sensors = 4;%4 % <------- CHANGE NUMBER OF ELEMENTS HERE 
...
%UPDate HERE till END
c = 1540; % Speed of sound in soft tissue, in m/s. 
for rx = 1:channel_data.N_elements
    m = x_element_position(rx);  % x-position of the element 'm'
    for ix = 1:scan.N_x_axis
        for iz = 1:scan.N_z_axis
            R = sqrt(z_pixels(ix, iz)^2 + (x_pixels(ix, iz) - m)^2);
            receive_delay(ix, iz, rx) = R / c;
        end
    end
...
psf_x_loc = 0.12/1000; % Find the x-location of the point scatter in m 
psf_z_loc = 24.32/1000; % FInd the z-location of the point scatter in m 
