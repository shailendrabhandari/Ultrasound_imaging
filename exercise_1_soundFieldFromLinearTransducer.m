...
rFocus = 15e-3;      % Focal radius, transducer [m]
...
figure(2); 
disp('Click on the -6 dB points at 10 mm depth');
[x_10mm, ~] = ginput(2);  % Capture two points along the -6 dB contour line at depth 40 mm
% Calculate the width (assuming x-values are in mm)
width_6dB_10mm = abs(x_10mm(1) - x_10mm(2));
% Display the calculated width
fprintf('The -6 dB beamwidth at 10 mm depth is %f mm\n', width_6dB_10mm);
% Compute theoretical beamwidth using Rindal's formula
D = num_elements * dx;
theta_6dB_theoretical_10mm = 1.21 * lambda / D;


% Calculate theoretical angle using Rindal's (1.8)formula

theta_6dB_theoretical = 1.21 * lambda / D;
theta_6dB_theoretical_deg = theta_6dB_theoretical * (180/pi);

% Compute farfield
R_farfield = D^2 /(4*lambda);

% Display results
disp(['Estimated Beamwidth at 10 mm: ', num2str(width_6dB_10mm)]);
disp(['Theoretical Beamwidth: ', num2str(theta_6dB_theoretical_10mm)]);
disp(['Farfield starts at: ', num2str(R_farfield)]);
disp('Click on the -6 dB points at 10 mm depth');
[x_10mm, y_10mm] = ginput(2);
disp(x_10mm);  % Display the captured x-values
disp(y_10mm);  % Display the captured y-values
